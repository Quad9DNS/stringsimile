...
It may be useful to implement a TTL-based cache and a Bloom filter (or maybe the "new hotness" of binary fuse filters? https://docs.rs/xorf/latest/xorf/)  of some sort to determine what string sets have already been compared so effort is not wasted. Many names like "www" or "mail" or others could be easily removed from the comparison set after being evaluated once (a quick check with real data proves this out - about 50% of all of the  first names in our UDR feed are duplicates, even in just 1 minute of data.)  I also suspect that type of duplication will happen along the entire path of the name in every zone cut - there will be lots of duplicates that should be quickly eliminated as already have failed/passed the tests since last config update, and avoiding doing the needless comparisons would be a huge win on CPU time.
