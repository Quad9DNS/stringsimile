...
fn basic_file_test() {
    ...
    for i in 0..input_data_parsed.len() {
        ...
        let stringsimile_results = results_row["stringsimile"]
            .as_object()
            .expect("Expected stringsimile to be a JSON object")["groups"]
            .as_array()
            .expect("Expected stringsimile groups to be a JSON array");

        // Expecting just a single result, because there is just a single string group
        assert_eq!(stringsimile_results.len(), 1);

        for res in stringsimile_results {
            let group = res
                .as_object()
                .expect("Expected result group to be a JSON object");
            assert!(group.contains_key("string_group_name"));
            assert!(group.contains_key("rule_sets"));
        }
    }
}
